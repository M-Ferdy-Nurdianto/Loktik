...
export const EventDetail = () => {
  ...
  const mapsQueryUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(event.name)}`;

  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8 text-left pb-28 sm:pb-16">
      {/* Event Header & Poster */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
        <div className="md:col-span-1 space-y-3">
          <Card variant="dark" className="p-2 shadow-2xl">
        ...
        <div className="md:col-span-2 space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 text-xs font-extrabold text-brand-blue">
            <div className="flex items-center space-x-2 bg-neutral-900/90 px-3 py-2 rounded-lg border border-neutral-800 text-brand-blue">
              <Calendar className="w-4 h-4 shrink-0 text-brand-blue" />
            ...
            <a
              ...
              className="flex items-center space-x-2 bg-neutral-900/90 px-3 py-2 rounded-lg border border-neutral-800 text-brand-blue hover:text-white transition-colors touch-press"
            >
              <MapPin className="w-4 h-4 shrink-0" />
              <span>PETUNJUK ARAH VENUE</span>
            </a>
          </div>

          <Card variant="dark" className="p-4 sm:p-5">
            <h3 className="font-extrabold text-xs uppercase text-brand-blue tracking-widest mb-2">DESKRIPSI ACARA</h3>
      ...
      <section className="space-y-4 sm:space-y-6 pt-2">
        <div className="border-b border-neutral-800 pb-2 flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-brand-blue font-bold uppercase">DIRECT TRANSAKSI</span>
        </div>
              ...
              <Card key={tier.id} variant="dark" hover className="flex flex-col justify-between space-y-4 p-4 sm:p-5 border border-neutral-800 hover:border-brand-blue/60 transition-colors">
                <div className="space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="text-base sm:text-lg font-extrabold uppercase text-white leading-tight">{tier.name}</h3>
                    <Badge variant={isSoldOut ? 'red' : 'blue'} className="shrink-0 text-[10px] sm:text-xs font-extrabold">
                      {isSoldOut ? 'HABIS' : isUnlimited ? 'OPEN PO' : `SISA ${tier.quota}`}
                    </Badge>
                  </div>
                  {tier.description && (
                    <p className="text-xs text-neutral-400 font-medium leading-relaxed">{tier.description}</p>
                  )}
                  <p className="text-lg sm:text-2xl font-black text-brand-blue tracking-tight font-mono">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-neutral-800">
                  ...
                  <div className="flex items-center space-x-2.5">
                    <button
                      ...
                      className="w-10 h-10 bg-neutral-800 text-white rounded-lg flex items-center justify-center hover:bg-brand-red disabled:opacity-30 active:scale-95 transition-transform touch-press"
                      aria-label="Kurangi tiket"
                    >
                      <Minus className="w-4 h-4 stroke-[2.5]" />
                    </button>
                    <span className="w-8 text-center font-extrabold text-lg text-white">{currentQty}</span>
                    <button
                      disabled={isSoldOut}
                      onClick={() => updateQuantity(tier.id, 1, tier.quota)}
                      className="w-10 h-10 bg-brand-blue text-black rounded-lg flex items-center justify-center hover:bg-[#009fb9] disabled:opacity-30 active:scale-95 transition-transform touch-press"
